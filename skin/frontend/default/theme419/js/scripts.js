jQuery(document).ready(function(){
	...
	if (jQuery('.container_24').width() < 450) {
		...
		jQuery('.box-collateral > h2').click(function(){
			...
			jQuery(this).parents('.box-collateral').find(' > .box-collateral-content').slideToggle("slow")
		});
	};
	...
	if (jQuery('.container_24').width() > 800) {
		jQuery('.header-button').not('.top-login').hover(
			function(){
				...
				jQuery(this).find('a').css({'background-color': '#F68694'}).parent().find('ul').toggle()/*css({'display':'block','height':'0'}).stop().animate({height:ListHeight, opacity: 1}, 200)*/
			},
...
jQuery(document).ready(function() {
	...
	jQuery('.banners-block a, .banner-list li a ').hover(function(){
	...
	var maxHeight = 0;
	function setHeight(column) {column = jQuery(column);
